...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-lg border-t border-white/60 shadow-[0_-8px_30px_rgba(0,0,0,0.1)] rounded-t-3xl pb-[env(safe-area-inset-bottom)]">
      <div className="grid grid-cols-3 h-[68px] max-w-lg mx-auto px-2">
        ...
        >
          <PlusCircle className={`w-5 h-5 ${activeTab === ('register-vendor' as any) ? 'text-[#3b82f6]' : ''}`} />
        ...
        >
          <div className="w-12 h-12 rounded-full bg-gradient-to-r from-[#06b6d4] to-[#3b82f6] hover:opacity-90 text-white flex items-center justify-center shadow-lg -mt-5 border-2 border-white">
            <Home className="w-6 h-6 stroke-[2.5]" />
          </div>
          <span className="text-[9px] sm:text-[10px] font-extrabold text-[#3b82f6] mt-1 text-center leading-[1.1] px-1">{t('homeTab')}</span>
        </button>
        ...
        >
          <div className="relative">
            ...
            {(currentRole === 'admin' || currentRole === 'volunteer') && pendingCount > 0 && (
              <span className="absolute -top-1 -right-2 bg-gradient-to-r from-[#06b6d4] to-[#3b82f6] text-white text-[9px] font-extrabold px-1.5 py-0.2 rounded-full border border-white">
                {pendingCount}
              </span>
            )}
          </div>
