  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 md:p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div className="md:max-w-xs">
        ...
        <button className="text-cyan-500 text-xs font-bold hover:underline">Explore More</button>
      </div>

      <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 md:pb-0 flex-1 justify-start md:justify-around">
        <TravelCard icon={<Plane className="w-8 h-8 text-cyan-500" />} title="Flight" subtitle="Powered By Easemytrip.com" onClick={() => onSelectCategory('travel')} />
        ...
        <TravelCard icon={<Car className="w-8 h-8 text-cyan-500" />} title="Car Rentals" subtitle="Drive Easy Anywhere" onClick={() => onSelectCategory('rent')} />
      </div>
    </div>
  );
};
...
const TravelCard = ({ icon, title, subtitle, onClick }: { icon: React.ReactNode, title: string, subtitle?: string, onClick: () => void }) => (
  <button onClick={onClick} className="flex flex-col items-center gap-3 min-w-[80px] shrink-0 group">
    <div className="w-16 h-16 bg-white border border-gray-200 rounded-2xl flex justify-center items-center group-hover:border-cyan-500lue-300 transition-colors shadow-sm">
